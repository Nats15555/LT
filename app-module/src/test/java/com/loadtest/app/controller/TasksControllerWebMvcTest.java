...
class TasksControllerWebMvcTest {
    ...
    @Test
    void queuePause_getAndPut() throws Exception {
        when(queuePauseService.getState()).thenReturn(new QueuePauseService.QueuePauseState(true, 3L));
        mockMvc.perform(get("/api/v1/loadtest/queue/pause"))
        ...
        mockMvc.perform(put("/api/v1/loadtest/queue/pause")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(Map.of("paused", false))))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.paused").value(false));
    }

    @Test
    void queuePause_putMissingPaused_returns400() throws Exception {
        mockMvc.perform(put("/api/v1/loadtest/queue/pause")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{}"))
    ...
    @Test
    void queuePause_putWithStringBoolean_returns200() throws Exception {
