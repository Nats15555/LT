...
FROM eclipse-temurin:21-jre-jammy AS execution
COPY --from=builder /workspace/execution-module/build/libs/*.jar /app.jar
EXPOSE 8082
ENTRYPOINT ["java", "-jar", "/app.jar"]

FROM eclipse-temurin:21-jre-jammy AS metrics-collector
COPY --from=builder /workspace/metrics-collector-module/build/libs/*.jar /app.jar
EXPOSE 8083
ENTRYPOINT ["java", "-jar", "/app.jar"]

FROM eclipse-temurin:21-jre-jammy AS summarization
